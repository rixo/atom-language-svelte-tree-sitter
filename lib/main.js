const packagesIndicatingTypeScriptUsage = [
  "language-typescript",
  "atom-typescript",
  "ide-typescript"
];
...
const getConfig = key => atom.config.get(`language-svelte-tree-sitter.${key}`);
...
const resolveDefaultHighlightSyntax = () => {
  const configValue = getConfig("defaultHighlightScript");
...
const resolveScriptNodeLang = node => {
  const lang = node
    ...
    .filter(
      attr => attr.descendantsOfType("attribute_name").pop()?.text === "lang"
    )
    .flatMap(attr =>
      attr.descendantsOfType("attribute_value").map(av => av.text)
    )
    .pop();
    ...
    language(node) {
      if (
        ...
        node.parent.children.some(
          sibling =>
            sibling.type === "attribute_name" && sibling.text === "style"
        )
      ) {
        return "css";
      }
